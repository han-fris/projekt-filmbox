...
seznamFilmuElement.textContent = '';

const seznamFilmu = filmy.forEach((popis) => {
  const seznamFilmuElement = document.querySelector('#seznam-filmu');
  seznamFilmuElement.innerHTML += `
    <div class="col">
      <div class="card">
        <img
          src=${popis.plakat.url}
          width=${popis.plakat.sirka}
          height=${popis.plakat.vyska}
          class="card-img-top"
          alt="plakát"
        />
        <div class="card-body">
          <h5 class="card-title">${popis.nazev}</h5>
          <p class="card-text">${popis.ochutnavka}</p>
          <a href="film.html#${popis.id}" class="btn btn-primary">Přehrát</a>
        </div>
      </div>
    </div>
    `;
